	...
	<body class="home blog">
		...
		<section class="container">
			<div class="excerpts-wrapper">
				<div class="excerpts">
					<h2 class="multi">&nbsp;&nbsp;<i class="fa fa-play-circle"></i> <span class="title"><?php  echo $this->value['VideoTypeName']; ?>列表页面</span></h2>
					<?php  $MYSQLVODS=(array)$this->value['VideoTypeJCSQL'];$VideoTypeId=$this->value['VideoTypeId'];$GetMb_page=$this->value['VideoTypePage']; $Count=9;$MYSQLVODS=PAGE($MYSQLVODS,$GetMb_page,$Count);  foreach($MYSQLVODS['source'] as $s){  ?>
					<article class="excerpt excerpt-c3">
						<a title="<font color='FF00DE'><?php 	echo  $s['d_name'] ?></font>" target="_blank" class="thumbnail" href="<?php 	echo  $Host1.'video_detail'.$Host2.$s['d_id'].$Host3.$VideoTypeId.$Host4; ?>">
							<img src="<?php 	echo  $s['d_pic'] ?>" data-src="<?php 	echo  $s['d_pic'] ?>" class="thumb"><i
							 class="fa fa-play-circle-o"></i></a>
						<a class="cat" href="<?php 	echo  $Host1.'video_detail'.$Host2.$s['d_id'].$Host3.$VideoTypeId.$Host4; ?>"><?php  echo $this->value['VideoTypeName']; ?></a>
						<h2><i class="fa fa-camera"></i> <a title="<font color='FF00DE'><?php 	echo  $s['d_name'] ?></font>" target="_blank" href="<?php 	echo  $Host1.'video_detail'.$Host2.$s['d_id'].$Host3.$VideoTypeId.$Host4; ?>">
