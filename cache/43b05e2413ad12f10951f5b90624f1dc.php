...
<html lang="zh-cn">
	<head>
		<title><?php echo $this->value['WebTitle']; ?>分类<?php  echo $this->value['PicTypeName']; ?>在线套图列表页面</title>
				...
				<div class="row">
					<div class="col-md-9 col-sm-8">
						<div class="well well-sm">
							<h2 style="font-size: 16px;margin: 5px 0;"><?php  echo $this->value['PicTypeName']; ?></h2>
						</div>
						<div class="row">
							<?php  $MYSQLVODS=(array)$this->value['PicTypeJCSQL'];$PicTypeId=$this->value['PicTypeId'];$GetMb_page=$this->value['PicTypePage']; $Count=30;$MYSQLVODS=PAGE($MYSQLVODS,$GetMb_page,$Count);  foreach($MYSQLVODS['source'] as $s){  ?>
							<div class="col-xs-6 col-sm-6 col-md-4 col-lg-4 col-video px-2">
								<div class="well well-sm">
									<a href="<?php 	echo  $Host1.'pic_detail'.$Host2.$s['a_id'].$Host3.$PicTypeId.$Host4; ?>">
										<div class="thumb-overlay" align="center">
											<img src="<?php 	echo  $s['a_remarks'] ?>" style="max-width: 100%;" title="<?php 	echo  $s['a_name'] ?>" alt="<?php 	echo  $s['a_name'] ?>" class="img-responsive ">
											<?php if(rand(0,1)){ ?>
											...
											<?php } ?>
											<div class="duration">
												<?php echo date('Y-m-d',strtotime($s['a_time'])) ?>
											</div>
											
										</div>
										<h3 class="video-title title-truncate m-t-5"><?php 	echo  $s['a_name'] ?></h3>
										<div class="videoBox-action">
											<span>浏览次数：<?php 	echo   rand(5, 10000); ?></span>
