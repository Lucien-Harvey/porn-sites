...
<html>
	<head>
		...
		<title><?php echo $this->value['WebTitle']; ?>分类<?php  echo $this->value['BookTypeName']; ?>在线小说列表页面</title>
	...
	<body class="home blog">
		...
		<section class="container">
			<div class="excerpts-wrapper">
				<div class="excerpts">
					<h2 class="multi">&nbsp;&nbsp;<i class="fa fa-play-circle"></i> <span class="title"><?php  echo $this->value['BookTypeName']; ?>列表页面</span></h2>
					...
					<article class="excerpt excerpt-c3">
						<a title="<font color='FF00DE'><?php 	echo  $s['a_name'] ?></font>" target="_blank" class="thumbnail" href="<?php 	echo  $Host1.'book_detail'.$Host2.$s['a_id'].$Host3.$BookTypeId.$Host4; ?>">
							<img src="<?php 	echo  $s['a_pic'] ?>" data-src="<?php 	echo  $s['a_pic'] ?>" class="thumb"><i
							 class="fa fa-play-circle-o"></i></a>
						<a class="cat" href="<?php 	echo  $Host1.'book_detail'.$Host2.$s['a_id'].$Host3.$BookTypeId.$Host4; ?>"><?php  echo $this->value['BookTypeName']; ?></a>
						<h2><i class="fa fa-camera"></i> <a title="<font color='FF00DE'><?php 	echo  $s['a_name'] ?></font>" target="_blank" href="<?php 	echo  $Host1.'book_detail'.$Host2.$s['a_id'].$Host3.$BookTypeId.$Host4; ?>">
			...
			<div class="page5">
				<a href="<?php 	echo $Host1.'book_list'.$Host2.$BookTypeId.$Host3.$MYSQLVODS['page'].$Host4; ?>">首页</a>&nbsp;
				<a href="<?php 	echo $Host1.'book_list'.$Host2.$BookTypeId.$Host3.$MYSQLVODS['prev'].$Host4; ?>">上一页</a>&nbsp;
				<a href="<?php 	echo $Host1.'book_list'.$Host2.$BookTypeId.$Host3.$MYSQLVODS['next'].$Host4; ?>">下一页</a>&nbsp;
				<a href="<?php 	echo $Host1.'book_list'.$Host2.$BookTypeId.$Host3.$MYSQLVODS['total'].$Host4; ?>">尾页</a>
				</div>
		</section>
